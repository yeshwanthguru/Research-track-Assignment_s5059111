...
/////to declare the publisher as global/////
ros::Publisher chatter_pub;
ros::ServiceClient client;
ros::Publisher pub2;
void subscriberCallback(const nav_msgs::Odometry::ConstPtr& pose_msg) ///// the information of Odometry/////
{
   ROS_INFO("Robot position: [%f, %f]", pose_msg->pose.pose.position.x, pose_msg->pose.pose.position.y);
   /////To Use the pose_msg to get the current position of Odometry/////
   geometry_msgs::Twist vel;
   ///// create the instance to call of service /////
...
int main(int argc, char **argv)
{
  ...
  ros::NodeHandle n;
  ///// set clients to-------connect------- the  service of random_target /////
  client = n.serviceClient<my_srv2::Random>("/random_target");
  ///// set publisher for sending-----info------of velocity to the stageros /////
